...
OUTPUT_DIR = os.path.join(BASE_DIR, 'noise_' + MODE)

if not os.path.isdir(OUTPUT_DIR):
    os.mkdir(OUTPUT_DIR)

save_file = open(os.path.join(BASE_DIR, "noise_" + MODE +".csv"), CSV_MODE, newline='')
writer = csv.writer(save_file)


if CSV_MODE == 'w':
    writer.writerow(["file", "detected", "sd"])
...
for root, dirs, files in os.walk(CHALLENGE_DIR):
    for file in files:
        ...
        if file.endswith("png") or file.endswith("jpg") or file.endswith("jpeg"):
            ...
            img = cv2.imread(os.path.join(root, file))
            for sd in noise_sd:
                img_c = gen_noise(img, sd)
                if MODE == 'sift':
                    detect_best_sift(templates, template_names, kp_t, des_t, img_c, file, 40, sd)
                elif MODE == 'surf':
                    detect_best_surf(templates, template_names, kp_t, des_t, img_c, file, 40, sd)
                elif MODE == 'orb':
                    detect_best_orb(templates, template_names, kp_t, des_t, img_c, file, 40, sd)
