...
OUTPUT_DIR = os.path.join(BASE_DIR, 'sort_' + MODE)

if not os.path.isdir(OUTPUT_DIR):
    os.mkdir(OUTPUT_DIR)

save_file = open(os.path.join(BASE_DIR, "sort_" + MODE +".csv"), CSV_MODE, newline='')
writer = csv.writer(save_file)


if CSV_MODE == 'w':
    writer.writerow(["file", "detected", "frame time"])
...
def show_result(img_t, img_c, temp_name, file_name,  algo, kp_t, kp_c, matches, good_matches, avg, time_start, time_end):
    detect = 'unknown'
    ...
    writer.writerow([file_name, detect, frame_time])
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    cv2.imwrite(os.path.join(output_dir, str(file_name)+'.jpg'), img_result)


def detect_best_orb(templates, template_names, kp_t, des_t, img_c, name, top, time_start, time_end):
    ...
    show_result(templates[0], img_c, template_names[0],  name, 'orb', kp_t[0], kp_c, all_matches[0], good_matches,
                avg[0], time_start, time_end)


def detect_best_surf(templates, template_names, kp_t, des_t, img_c, name, top, time_start, time_end):
...
def detect_best_sift(templates, template_names, kp_t, des_t, img_c, name, top, time_start, time_end):
...
def gen_template_surf(templates, template_names):
    ...
    for img_t, file in zip(templates, template_names):
        ...
        kp.append(kp_t)
        des.append(des_t)
    return kp, des, time_start, time_end
...
time_start, time_end = [], []
if MODE == 'sift':
    kp_t, des_t, time_start, time_end = gen_template_sift(templates, template_names)
elif MODE == 'surf':
    kp_t, des_t, time_start, time_end = gen_template_surf(templates, template_names)
elif MODE == 'orb':
    kp_t, des_t, time_start, time_end = gen_template_orb(templates, template_names)
...
for root, dirs, files in os.walk(CHALLENGE_DIR):
    ...
    for file in files:
        ...
        if file.endswith("png") or file.endswith("jpg") or file.endswith("jpeg"):
            ...
            if MODE == 'sift':
                detect_best_sift(templates, template_names, kp_t, des_t, img_c, file, 40, time_start, time_end)
            elif MODE == 'surf':
                detect_best_surf(templates, template_names, kp_t, des_t, img_c, file, 40, time_start, time_end)
            elif MODE == 'orb':
                detect_best_orb(templates, template_names, kp_t, des_t, img_c, file, 40, time_start, time_end)
